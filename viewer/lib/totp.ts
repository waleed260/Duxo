...
// ─── Encryption lives on the server now ───
//
// encryptSecret/decryptSecret used to be here, running PBKDF2 with the user's
// uid as the password — and the uid is the path the ciphertext is stored at
// (`users/{uid}`), so anyone who could read the document already held every
// input needed to derive the key. It was obfuscation, not confidentiality
// against the one compromise it existed for.
//
// They now live in lib/totp-server.ts, keyed by HKDF from a master secret
// that never leaves the server environment, and are reached through
// /api/totp/{setup,activate,verify}. The plaintext secret no longer exists in
// a page context after enrolment.
